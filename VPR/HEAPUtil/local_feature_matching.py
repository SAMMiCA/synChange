...
from NetVLAD.dataset1 import get_whole_val_set
...
parser.add_argument('--dataset', type=str, default='berlin', 
        help='Dataset to use', choices=['pittsburgh','oxford',
                                        'darkzurich','nordland',
                                        'tokyo247','berlin',
                                        'gardens_point','tb_places',
                                        'changesim', 'gsv', 'tsunami', 'vlcmucd'])
parser.add_argument(
...
def write_match_output(predictions, q_path, r_path, ds_name, K):
    outfile = 'HEAPUtil_' + ds_name + '_predictions.txt'
    print('Writing matching results to', outfile)
    with open(outfile, 'w') as kap_out:
        kap_out.write('# model: HEAPUtil \n')
        kap_out.write(f'# Top {K} results \n')
        for q_idx in range(len(predictions)):
            qp = q_path[q_idx]
            for r_idx in range(K):
                rp = r_path[predictions[q_idx, r_idx]]
                kap_out.write(qp + ', ' + rp + '\n')
...
if __name__ == "__main__":
    ...
    np.savetxt(os.path.join(output_dir, 'lfm_' + opt.dataset + '_recalls.txt'), recall_at_n)
    write_match_output(predictions.astype(int), 
                       dataset.dbStruct.qImage, dataset.dbStruct.dbImage, opt.dataset, max(n_values))
    ...
    if opt.viz:
        ...
        for i in tqdm(range(0, len(query_image_paths[::20])), desc='Visualizing...'):
