...
if __name__ == "__main__":
    ...
    if opt.save_viz:
        ...
        print(f'# of image paths: {len(image_paths)}')
        
        for i in tqdm(np.arange(0, len(image_paths))[::20], desc='Visualizing...'):
            ...
            plt.axis('off')
            plt.imshow(color.label2rgb(db_cluster_masks[i].astype(int), img, colors=new_colors, bg_label=-1, alpha=0.5)) # PS
            ...
            plt.axis('off')
            plt.imshow(color.label2rgb(es_binary_mask.astype(int), img, bg_label=1, alpha=0.5)) # ES
            plt.tight_layout(pad=0)
