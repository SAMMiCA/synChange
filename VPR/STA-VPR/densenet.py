...
def densenetFeature(H_path,aC_path):
    ...
    for i in tqdm(range(len(H_path)),sta):
        img = cv2.imread(H_path[i])
    ...
    for i in tqdm(range(len(aC_path)),sta):
        img = cv2.imread(aC_path[i])
