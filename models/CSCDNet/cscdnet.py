...
from models.CSCDNet import init
...
class CSCDNet(nn.Module):
    def __init__(self, inc=6, outc=2, corr=True, pretrained=True):
        super(CSCDNet, self).__init__()
        self.num_class = outc
        self.corr = corr
        ...
        if self.corr is True:
            ...
            self.dec_corr2 = SpatialCorrelationSampler(
                                kernel_size=1,
                                patch_size=21,
                                stride=1,
                                padding=0,
        ...
        if self.corr is True:
            ...
            self.dec_corr1 = SpatialCorrelationSampler(
                                kernel_size=1,
                                patch_size=21,
                                stride=1,
                                padding=0,
        ...
        if pretrained is True:
            self.load_net_param()
    def forward(self, im_target, im_source,
                im_target_256=None, im_source_256=None,disable_flow=None):
        x1, x2 = im_source, im_target
        ...
        if self.corr is True:
            #cor = correlation.FunctionCorrelation(tensorFirst=enc1_f3, tensorSecond=enc2_f3)
            cor = self.dec_corr2(enc1_f3, enc2_f3)
            cor = cor.view(enc1_f3.shape[0],-1,enc1_f3.shape[2],enc1_f3.shape[3])
        ...
        dec = self.unpool(dec)
        if self.corr is True:
            # cor = correlation.FunctionCorrelation(tensorFirst=enc1_f1, tensorSecond=enc2_f1)
            cor = self.dec_corr1(enc1_f1, enc2_f1)
            cor = cor.view(enc1_f1.shape[0],-1,enc1_f1.shape[2],enc1_f1.shape[3])
            cor = self.corr_activation(cor)
