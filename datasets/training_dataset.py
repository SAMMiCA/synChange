...
class HomoAffTps_Dataset(Dataset):
    ...
    def __init__(self,
        ...
        #self.coco_transform = A.Compose([A.Resize
        ...
        self.coco_data = CocoDetectionCP(
            'data_coco/train2014/',
            'data_coco/annotations/instances_train2014.json'
        )
    ...
    def __getitem__(self, idx):
        ...
        return_dict.update({'img': {'ref': {}, 'query': {}},
                            'mask': {'ref': {}, 'query': {}}})
        ...
        if transform_type == 0 or transform_type == 1:
            ...
            for change_type in ['new','missing','replaced','rotated']:
                ...
                if change_type == 'new':
                    ...
                    src_img_with_mask, tgt_img_with_mask = self.paste_from_cropped_obj(original_img,obj_src_dict,obj_tgt_dict)
                    # Source img
                    img_src_crop = \
                        self.transform_image(src_img_with_mask[None,...],
                                             self.H_OUT,
                ...
                return_dict['img']['ref'][change_type] = img_src_crop
                return_dict['img']['query'][change_type] = img_tgt_crop
                return_dict['mask']['ref'][change_type] = mask_src_crop
                return_dict['mask']['query'][change_type] = mask_tgt_crop
        ...
        # Homography transformation
            ...
            # Paste object thumbnail from COCO dataset
            pasted, _ = self.copy_paste(img_src_orig,idx,num_min_pixel=10000,num_max_pixel=100000)

            # get a central crop:
            img_src_crop, x1_crop, y1_crop = center_crop(pasted['image'], self.W_OUT)
            ...
            for change_type in ['new','missing','replaced','rotated']:
                ...
                if change_type == 'new':
                    ...
                    src_img_with_mask, tgt_img_with_mask = self.paste_from_cropped_obj(img_src_orig,obj_src_dict,obj_tgt_dict)
                    ...
                    theta_extra = self.prepare_extra_theta(transform_type=random.randint(0,1))
                    img_tgt_crop = \
                        self.transform_image(obj_img_with_mask[None, ...],
                                             src_h,
                                             src_w,
                    ...
                    src_img_with_mask, tgt_img_with_mask = self.paste_from_cropped_obj(img_src_orig, obj_src_dict,
                                                                                       obj_tgt_dict)
        ...
        if self.get_flow:
            ...
            return {'source_image': torch.zeros(3,520,520),
                    'target_image': torch.zeros(3,520,520),
        ...
        while 1:
            try:
                ...
                tf_crop_resize = A.Compose([
                    ...
                    A.Resize(height=min_resize_length, width=min_resize_length),
                ...
                # Do copy-paste
                pasted = self.copy_paste_transform_prob50(image=img_src_orig,
    ...
    def paste_from_cropped_obj(self,original_img, obj_src_dict, obj_tgt_dict):
        ...
        (tgt_h, tgt_w, c) = obj_tgt_dict['image'].shape
        orig_cx, orig_cy = original_img.shape[2] // 2, original_img.shape[1] // 2
        x_range = [orig_cx - 520 // 2, orig_cx + 520 // 2 - src_w]
        y_range = [orig_cy - 520 // 2, orig_cy + 520 // 2 - src_h]
        x = random.randint(x_range[0], x_range[1])
        y = random.randint(y_range[0], y_range[1])
        src_img = original_img.clone()
        src_img[:, y:y + src_h, x:x + src_w] = \
    ...
    def viz_change_aug(self,img_src_crop,img_tgt_crop,mask_src_crop,mask_tgt_crop,
                       change_type,transform_type,idx,show=False,save=True):
        plot_idx = 141
        plt.clf()
        plt.close()
